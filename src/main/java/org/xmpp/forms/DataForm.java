 ...
 * </ul>
 * <p/>
 * In case the form represents a search, the report will be structured in
 * columns and rows. Use {@link #addReportedField(String,String,FormField.Type)}
 * to set the columns of the report whilst the report's rows can be configured
 * using {@link #addItemFields(Map)}.
 * 
...
public class DataForm extends PacketExtension {
	 ...
	 * The date representation is expected to be in the UTC GMT+0 format.
	 * 
	 * @param date
	 *            date representation in the UTC GMT+0 format.
	 * @return the Date obtained by parsing the specified date representation.
	 * @throws ParseException
	 *             if an error occurs while parsing the date representation.
	 */
	...
	/**
	 * Returns the String representation of an Object to be used as a field
	 * value.
	 * 
	 * @param object
	 *            the object to encode.
	 * @return the String representation of an Object to be used as a field
	 *         value.
	 */
	...
	/**
	 * Returns an unmodifiable list of instructions that explain how to fill out
	 * the form and what the form is about. The dataform could include multiple
	 * instructions since each instruction could not contain newlines
	 * characters.
	 * 
	 ...
	 * characters.
	 * <p>
	 * Nothing will be set, if the provided argument is <tt>null</tt> or an
	 * empty String.
	 * 
	 * @param instruction
	 ...
	 *            Optional parameter.
	 * @param type
	 *            an indicative of the format for the data. Optional parameter.
	 * @param label
	 *            the label of the question. Optional parameter.
	 * @return the newly created field.
	 */
	public FormField addField(final String variable, final String label, final FormField.Type type) {
	 ...
	 * Each field represents a column in the report. The order of the columns in
	 * the report will honor the sequence in which they were added.
	 * 
	 * @param variable
	 *            variable name of the new column. This value will be used in
	 *            {@link #addItemFields} when adding reported items.
	 * @param label
	 *            label that corresponds to the new column. Optional parameter.
	 * @param type
	...
	/**
	 * Adds a new row of items of reported data. For each entry in the
	 * <tt>fields</tt> parameter a <tt>field</tt> element will be added to the
	 * <item> element. The variable of the new <tt>field</tt> will be the key of
	 * the entry. The new <tt>field</tt> will have several values if the entry's
	 * value is a {@link Collection}. Since the value is of type {@link Object}
	 * it is possible to include any type of object as a value. The actual value
	 * to include in the data form is the result of the {@link #encode(Object)}
	 * method.
	 * 
	 * @param fields
	...
	public enum Type {
		/**
		 * The forms-processing entity is asking the forms-submitting entity to
		 * complete a form.
		 */
		form,

		/**
		 * The forms-submitting entity is submitting data to the
		 * forms-processing entity.
		 */
		submit,

		/**
		 * The forms-submitting entity has cancelled submission of data to the
		 * forms-processing entity.
		 */
		cancel,

		/**
		 * The forms-processing entity is returning data (e.g., search results)
