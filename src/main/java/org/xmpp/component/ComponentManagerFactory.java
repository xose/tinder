...
/**
 * Factory to get a ComponentManager implementation. The ComponentManager
 * implementation used will determined in the following way:
 * <ul>
 * 
 * <li>An external process can set the ComponentManager using
 * {@link #setComponentManager(ComponentManager)}.
 * <li>If the component manager is <tt>null</tt>, the factory will check for the
 * Java system property "whack.componentManagerClass". The value of the property
 * should be the fully qualified class name of a ComponentManager implementation
 * (e.g. com.foo.MyComponentManager). The class must have a default constructor.
 * </ul>
 * 
 * @author Matt Tucker
 */
public class ComponentManagerFactory {
	...
	public static synchronized ComponentManager getComponentManager() {
		...
		final String className = System.getProperty("whack.componentManagerClass");
		if (className != null) {
			try {
	 ...
	 */
	public static void setComponentManager(final ComponentManager manager) {
