 ...
 * primary server domain (<tt>example.com</tt> in the example above);
 * sub-domains are only used for routing within the XMPP server.
 * 
 * @author Matt Tucker
 */
public interface Component {
	...
	/**
	 * Initializes this component with a ComponentManager and the JID that this
	 * component is available at (e.g. <tt>service.example.com</tt>). If a
	 * ComponentException is thrown then the component will not be loaded.
	 * <p>
	 * 
	 * The initialization code must not rely on receiving packets from the
	 * server since the component has not been fully initialized yet. This means
	 * that at this point the component must not rely on information that is
	 * obtained from the server such us discovered items.
	 * 
	 * @param jid
	 *            the XMPP address that this component is available at.
	 * @param componentManager
	 *            the component manager.
	 * @throws ComponentException
	 *             if an error occured while initializing the component.
	 */
	public void initialize(JID jid, ComponentManager componentManager) throws ComponentException;

	/**
	 * Notification message indicating that the component will start receiving
	 * incoming packets. At this time the component may finish pending
	 * initialization issues that require information obtained from the server.
	 * <p>
	 * 
